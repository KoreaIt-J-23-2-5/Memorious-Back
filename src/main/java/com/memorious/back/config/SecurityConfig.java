...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                .antMatchers("/api/auth/**") // 엔드포인트 지정. "/auth/**" 주소 이면.. 통과
...
//                .authenticationEntryPoint(principalEntryPoint)
                .and()
                .oauth2Login()
                .loginPage("http://localhost:3000/auth/oauth2/signin") //FE에서 로그인 화면
                ...
                .userInfoEndpoint() //로그인한 네이버 계정 정보 가져옴 -> 서비스로 넘김
                //userService : authmanager와 같은 역할 (authentication 객체를 만들어줌
